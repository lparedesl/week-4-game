$(document).ready(function() {
	...
	function reset() {
		...
		// losses = 0,
		n = -2;
		$("[id=" + chosenCharacter.attr("charname") + "_life]").text(chosenCharacterLife);
		$("[id=" + enemy.attr("charname") + "_life]").text(enemyLife);
		...
		for (var i = 0; i < characters.length; i++) {
			position = $("[charname=" + characters[i] + "]").position();
			moveLeft = 328 + 228 * i - position.left;
			moveTop = 400 - position.top;
			$("[charname=" + characters[i] + "]").attr("class", "character hvr-pulse-grow");
			$("[charname=" + characters[i] + "]").html("<img src=\"assets/images/" + characters[i] + ".png\">\n<p id=\"" + characters[i] + "\">" + characters[i] + "</p>\n<p id=\"" + characters[i] + "_life\">" + lives[i] + "</p>");

			$("[charname=" + characters[i] + "]").animate({
				height: "40%"
			});
			$("[charname=" + characters[i] + "]").animate({
				top: "+=" + moveTop,
	...
	$(".character").on("click", function() {
		if (gameStarted === false && gameEnded === false) {
			...
			var indexChosenCharacter = characters.indexOf(chosenCharacter.attr("charname"));
			...
			// Move enemies
			for (var i = 0; i < characters.length; i++) {
				$("[charname=" + characters[i] + "]").removeClass("hvr-pulse-grow");
				$("[charname=" + characters[i] + "]").addClass("hvr-pulse-shrink");
				position = $("[charname=" + characters[i] + "]").position();
	...
	$(".character").on("click", function() {
		if (n > -1 && gameStarted === true && fightStarted === false && gameEnded === false) {
			...
			if (n > 0) {
				(enemies[(n-1)]).html("");
				newChosenCharacterLife = chosenCharacterLife;
				$("[id=" + chosenCharacter.attr("charname") + "_life]").text(newChosenCharacterLife);
			}
			...
			$("[id=" + enemy.attr("charname") + "], [id=" + enemy.attr("charname") + "_life]").animate({
				"font-size": "20px"
			});
			$("[id=" + enemy.attr("charname") + "], [id=" + enemy.attr("charname") + "_life]").css({
				top: 380,
				left: 0,
			});
			...
			var indexChosenEnemy = characters.indexOf(enemy.attr("charname"));
	...
	document.onkeyup = function(event) {
		if (fightStarted === true && event.key === " ") {
			...
			newEnemyLife -= attack;
			$(".instructions").html("<p>You attacked "+ enemy.attr("charname") + " for " + attack + " damage</p>\n<p>" + enemy.attr("charname") + " attacked you back for " + enemyAttack + " damage</p>");
			...
			// User losses
				...
				$("[id=" + chosenCharacter.attr("charname") + "_life]").text(0);
			...
			if (newEnemyLife > 0) {
				$("[id=" + enemy.attr("charname") + "_life]").text(newEnemyLife);
			}

			// User wins
			else {
				$("[id=" + enemy.attr("charname") + "_life]").text(0);
				if (enemies.length === 4) {
					...
					$(".instructions").html("<p>You defeated " + enemy.attr("charname") + " .</p>\n<p>Your strength has been increased by 40 percent</p>");
				}
				fightEnded = true;
				fightStarted = false;
			}
